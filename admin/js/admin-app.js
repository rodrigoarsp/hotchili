...
class AdminApp {
                                ...
                                <div id="prod-cropper-section" class="p-4 bg-surface-card rounded-xl border border-primary/30 space-y-3">
                                    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 border-b border-surface-border pb-2">
                                        ...
                                        <div class="flex items-center gap-2">
                                            <select id="prod-cropper-preset" class="px-2.5 py-1 bg-surface rounded-lg border border-surface-border text-xs text-on-surface focus:border-primary focus:outline-none">
                                                <option value="fashion_portrait">Retrato de Moda (4:5)</option>
                                                <option value="square">Quadrado (1:1)</option>
                                            </select>
                                            <button type="button" id="prod-cropper-reset-btn" class="p-1 hover:bg-surface rounded text-primary text-xs flex items-center gap-1 border border-surface-border" title="Centralizar Imagem">
                                    ...
                                    <div class="flex justify-center items-center py-2">
                                        <div id="prod-canvas-wrapper" class="relative overflow-hidden rounded-xl border-2 border-primary shadow-2xl bg-black flex items-center justify-center cursor-grab active:cursor-grabbing select-none" style="width: 240px; height: 300px;">
                                            ...
                                            <div class="absolute inset-0 border border-primary/20 pointer-events-none grid grid-cols-3 grid-rows-3 opacity-40">
                                    ...
                                    <div class="flex items-center gap-3 pt-1">
                                        ...
                                        <span class="material-symbols-outlined text-on-surface-muted text-sm">zoom_in</span>
                                        <span id="prod-zoom-label" class="text-xs font-mono text-primary w-10 text-right">100%</span>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    renderHeroModal() {
        return `
            <div id="hero-modal" class="fixed inset-0 z-50 bg-black/80 backdrop-blur-xs flex items-center justify-center p-4 hidden">
                <div class="bg-surface-card border border-surface-border rounded-2xl w-full max-w-2xl overflow-hidden shadow-2xl">
                    <div class="p-6 border-b border-surface-border flex items-center justify-between">
                        ...
                        <div class="p-4 bg-surface rounded-xl border border-surface-border space-y-3">
                            <div class="flex items-center justify-between">
                                <span class="text-xs font-bold text-primary uppercase tracking-wider flex items-center gap-1.5">
                                    ...
                                    Banner Panorâmico &amp; Enquadramento
                                </span>
                            ...
                            <div id="hero-upload-panel" class="space-y-2">
                                <div id="hero-dropzone" class="border-2 border-dashed border-primary/30 hover:border-primary/70 bg-surface-card/60 hover:bg-surface-card transition-all rounded-xl p-4 text-center cursor-pointer relative group">
                                    ...
                                    <div class="space-y-1">
                                        ...
                                        <p class="text-xs font-bold text-on-surface">Enviar imagem do computador</p>
                                        <p class="text-[10px] text-on-surface-muted">Suporta JPG, PNG e WEBP</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div id="hero-url-panel" class="space-y-2 hidden">
                                <div class="flex gap-2">
                                    <input id="modal-hero-image" type="url" class="flex-1 px-4 py-2 bg-surface-card rounded-lg border border-surface-border text-xs text-on-surface focus:border-primary focus:outline-none font-mono" placeholder="https://exemplo.com/banner.jpg"/>
                                    <button type="button" id="hero-load-url-btn" class="px-4 py-2 bg-surface-card hover:bg-surface border border-surface-border text-xs text-primary font-bold rounded-lg transition-colors">
                                        Carregar
                                    </button>
                                </div>
                            </div>
                            ...
                            <div id="hero-cropper-section" class="p-3 bg-surface-card rounded-xl border border-primary/30 space-y-3">
                                <div class="flex items-center justify-between border-b border-surface-border pb-2">
                                    ...
                                    <button type="button" id="hero-cropper-reset-btn" class="p-1 hover:bg-surface rounded text-primary text-xs flex items-center gap-1 border border-surface-border">
                                        <span class="material-symbols-outlined text-sm">restart_alt</span> Reset
                                    </button>
                                </div>

                                <div class="flex justify-center items-center py-2">
                                    <div id="hero-canvas-wrapper" class="relative overflow-hidden rounded-xl border-2 border-primary shadow-2xl bg-black flex items-center justify-center cursor-grab active:cursor-grabbing select-none w-full" style="max-width: 480px; height: 200px;">
                                        <canvas id="hero-crop-canvas" width="1920" height="800" class="w-full h-full object-cover pointer-events-none"></canvas>
                                    </div>
                                </div>

                                <div class="flex items-center gap-3 pt-1">
                                    <span class="material-symbols-outlined text-on-surface-muted text-sm">zoom_out</span>
                                    <input id="hero-zoom-slider" type="range" min="1" max="3" step="0.02" value="1" class="flex-1 accent-primary cursor-pointer"/>
                                    <span class="material-symbols-outlined text-on-surface-muted text-sm">zoom_in</span>
                                    <span id="hero-zoom-label" class="text-xs font-mono text-primary w-10 text-right">100%</span>
                                </div>
                            </div>
                        </div>
    ...
    setupProductCropper() {
        ...
        const zoomSlider = document.getElementById('prod-zoom-slider');
        const zoomLabel = document.getElementById('prod-zoom-label');
        const presetSelect = document.getElementById('prod-cropper-preset');
        const resetBtn = document.getElementById('prod-cropper-reset-btn');
        ...
        const redrawCanvas = () => {
            ...
            if (!this.prodCrop.imgObj) return;

            const img = this.prodCrop.imgObj;
            ...
            const currentW = baseW * this.prodCrop.scale;
            const currentH = baseH * this.prodCrop.scale;

            const centerX = (canvas.width - currentW) / 2 + this.prodCrop.offsetX;
            const centerY = (canvas.height - currentH) / 2 + this.prodCrop.offsetY;

            ctx.drawImage(img, centerX, centerY, currentW, currentH);
        ...
        if (presetSelect) {
            presetSelect.addEventListener('change', (e) => {
                ...
                if (val === 'square') {
                    ...
                    wrapper.style.width = '240px'; wrapper.style.height = '240px';
                } else {
                    canvas.width = 800; canvas.height = 1000;
                    wrapper.style.width = '240px'; wrapper.style.height = '300px';
                }
                this.prodCrop.offsetX = 0;
                this.prodCrop.offsetY = 0;
                redrawCanvas();
            });
        }
        ...
        if (zoomSlider) {
            zoomSlider.addEventListener('input', (e) => {
                ...
                if (zoomLabel) zoomLabel.textContent = `${Math.round(this.prodCrop.scale * 100)}%`;
                redrawCanvas();
            });
        }
        ...
        wrapper.addEventListener('pointerdown', (e) => {
            ...
            this.prodCrop.startY = e.clientY - this.prodCrop.offsetY;
        ...
        const endDrag = (e) => {
            this.prodCrop.isDragging = false;
    ...
    loadProductImageToCropper(src) {
        const img = new Image();
        img.crossOrigin = 'Anonymous';
        img.onload = () => {
            this.prodCrop.imgObj = img;
            this.prodCrop.imgSrc = src;
            this.prodCrop.offsetX = 0;
            this.prodCrop.offsetY = 0;
            this.prodCrop.scale = 1;
            const zoomSlider = document.getElementById('prod-zoom-slider');
            ...
            if (zoomSlider) zoomSlider.value = 1;
            if (zoomLabel) zoomLabel.textContent = '100%';
            if (this.prodCrop.redraw) this.prodCrop.redraw();
        };
        img.src = src;
    }
    ...
    setupHeroCropper() {
        ...
        const canvas = document.getElementById('hero-crop-canvas');
        const zoomSlider = document.getElementById('hero-zoom-slider');
        const zoomLabel = document.getElementById('hero-zoom-label');
        const resetBtn = document.getElementById('hero-cropper-reset-btn');
        ...
        const redrawCanvas = () => {
            ...
            if (!this.heroCrop.imgObj) return;

            const img = this.heroCrop.imgObj;
            const targetRatio = canvas.width / canvas.height;
            const imgRatio = img.width / img.height;
            ...
            const currentW = baseW * this.heroCrop.scale;
            const currentH = baseH * this.heroCrop.scale;

            const centerX = (canvas.width - currentW) / 2 + this.heroCrop.offsetX;
            const centerY = (canvas.height - currentH) / 2 + this.heroCrop.offsetY;

            ctx.drawImage(img, centerX, centerY, currentW, currentH);

            try {
                this.heroImageResult = canvas.toDataURL('image/webp', 0.85);
            } catch (e) {
                this.heroImageResult = canvas.toDataURL('image/jpeg', 0.85);
            }
        };

        this.heroCrop.redraw = redrawCanvas;

        if (resetBtn) {
            resetBtn.addEventListener('click', () => {
                this.heroCrop.offsetX = 0;
                this.heroCrop.offsetY = 0;
                this.heroCrop.scale = 1;
                if (zoomSlider) zoomSlider.value = 1;
                if (zoomLabel) zoomLabel.textContent = '100%';
                redrawCanvas();
            });
        ...
        if (zoomSlider) {
            zoomSlider.addEventListener('input', (e) => {
                this.heroCrop.scale = parseFloat(e.target.value);
                if (zoomLabel) zoomLabel.textContent = `${Math.round(this.heroCrop.scale * 100)}%`;
                redrawCanvas();
            });
        }

        wrapper.addEventListener('pointerdown', (e) => {
            this.heroCrop.isDragging = true;
            this.heroCrop.startX = e.clientX - this.heroCrop.offsetX;
            this.heroCrop.startY = e.clientY - this.heroCrop.offsetY;
            wrapper.setPointerCapture(e.pointerId);
        });

        wrapper.addEventListener('pointermove', (e) => {
            if (!this.heroCrop.isDragging) return;
            const scaleFactor = canvas.width / wrapper.clientWidth;
            this.heroCrop.offsetX = (e.clientX - this.heroCrop.startX) * scaleFactor;
            this.heroCrop.offsetY = (e.clientY - this.heroCrop.startY) * scaleFactor;
            redrawCanvas();
        });

        const endDrag = (e) => {
            this.heroCrop.isDragging = false;
            try { wrapper.releasePointerCapture(e.pointerId); } catch (err) {}
        };
        wrapper.addEventListener('pointerup', endDrag);
        wrapper.addEventListener('pointercancel', endDrag);
    ...
    loadHeroImageToCropper(src) {
        const img = new Image();
        img.crossOrigin = 'Anonymous';
        img.onload = () => {
            this.heroCrop.imgObj = img;
            this.heroCrop.imgSrc = src;
            this.heroCrop.offsetX = 0;
            this.heroCrop.offsetY = 0;
            this.heroCrop.scale = 1;
            const zoomSlider = document.getElementById('hero-zoom-slider');
            ...
            if (zoomSlider) zoomSlider.value = 1;
            if (zoomLabel) zoomLabel.textContent = '100%';
            if (this.heroCrop.redraw) this.heroCrop.redraw();
        };
        img.src = src;
    }
    ...
    bindEvents() {
        ...
        const prodLoadUrlBtn = document.getElementById('prod-load-url-btn');
        ...
        const heroLoadUrlBtn = document.getElementById('hero-load-url-btn');
        ...
        if (heroFileInput) {
            heroFileInput.addEventListener('change', (e) => {
                if (e.target.files && e.target.files[0]) {
                    const reader = new FileReader();
                    reader.onload = (evt) => {
                        this.loadHeroImageToCropper(evt.target.result);
                    };
                    reader.readAsDataURL(e.target.files[0]);
                }
            });
        ...
        if (heroLoadUrlBtn && heroUrlInput) {
            heroLoadUrlBtn.addEventListener('click', () => {
                const url = heroUrlInput.value.trim();
                if (url) this.loadHeroImageToCropper(url);
            });
        }
        ...
        document.getElementById('save-hero-form')?.addEventListener('submit', async (e) => {
            ...
            submitBtn.disabled = true;
            submitBtn.innerHTML = `<span>Salvando Banner...</span>`;

            let finalHeroImage = this.heroImageResult || document.getElementById('modal-hero-image')?.value;
    ...
    openHeroModal(key) {
        ...
        const heroImg = hero.image_url || hero.imageUrl || 'https://lh3.googleusercontent.com/aida-public/AB6AXuChO5ac7GM05feevK3AKK8ckQsGI8zyLHcWYSqe_79EJZdlx2wmGqC0y2R5n77qB43BCg8ZaumkWAFY-A4K0FwYdpstPtOZjV46hdS0LsCEGjlhKsFbZNKQ2ARh25p94SEXswwCKW0I-4HmyMCeafYQNTl4ip2xdVkoCVGNnv_P04FItAfYej34n9I4C63sDpmC_8_psqPCjX9kK4BnHkcrXq_i7I7Dx437I6J2xxT6FgW-6WFy652D';
        document.getElementById('modal-hero-image').value = heroImg;
        this.heroImageResult = heroImg;

        if (title) title.textContent = `Editar Hero: ${key.toUpperCase()}`;
        modal.classList.remove('hidden');

        setTimeout(() => {
            this.loadHeroImageToCropper(heroImg);
        }, 50);
    }
